<?php
    ...
    $obj = new PDOMySQL();
    $sql = "update " . DB_NAME . ".meeting set startDate = ?, "
            . "endDate = ?, location = ?, event = ?,  category = ? where initiate = ? and meetingID = ?";
    $args = array($start, $end, $location, $event, $category, $id, $meetingID);
    $obj->query($sql, $args);
    
    header("Location: ../../welcome.php?notification=Reminder+edited+successfully!");
